...
/**
 * is_chain - determine if current char in buffer is a chain delimiter
...
/**
 * replace_alias - put in place an aliases in the tokenized str
 * @info: the ptr struct
...
int replace_vars(info_t *info)
{
	...
	for (l = 0; info->argv[l]; l++)
	{
		...
		{
			replace_string(&(info->argv[l]), _strdup(convert_number(info->status, 10, 0)));
			continue;
		}
		if (!_strcmp(info->argv[l], "$$"))
		{
			replace_string(&(info->argv[l]), _strdup(convert_number(getpid(), 10, 0)));
			continue;
		}
		node = node_starts_with(info->env, &info->argv[l][1], '=');
		if (node)
		{
			replace_string(&(info->argv[l]), _strdup(_strchr(node->str, '=') + 1));
			continue;
		}
